  ...
  public:
    ...
    void sample_goals()
    {
      ...
      {                                                                            // 20%
        ...
        delta_max_speed = 0.1 + 0.1 * Eigen::VectorXd::Random(1)[0]; // 0.1-0.2 
        delta_ang_speed = 0.3 + 0.1 * Eigen::VectorXd::Random(1)[0]; // 0.3-0.4
      ...
      {                                                                  // 90%
        ...
        delta_max_speed = 0 + 0.05 * Eigen::VectorXd::Random(1)[0]; // 
        delta_ang_speed = 0.1 + 0.05 * Eigen::VectorXd::Random(1)[0]; // 0.1 ~ 0.15
...
//    }
    ...
    double compute_angular_reward()
    {
      double r = 0.;
      double forward_r = adaptiveForwardVelRewardCoeff_ * std::min(max_speed, bodyLinearVel_[0]);
      double angular_r = adaptiveAngularVelRewardCoeff_ * std::min(ang_speed, bodyAngularVel_[2]);  // yaw speed -> and_speed

      r += forward_r;
      r += angular_r;
      r += -abs(bodyLinearVel_[1]); // dont go fast
      r += (alive_bonus * 1. / (2. - (double)isSlope));
      return r;
    }
    ...
    float step(const Eigen::Ref<EigenVec> &action_vec) final
    {
      ...
      forwardReward = compute_angular_reward();
