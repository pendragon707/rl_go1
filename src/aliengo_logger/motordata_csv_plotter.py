...
def read_last_n_lines_with_tail(filename, n=0, delimiter=','):
    """ Return DataFrame with n last strings from file. Uses linux tail.
    Args:
        filename (str): 
        n (int): number of tail strings, if n=0 read all file
...
def build_graph_from_csv(csvfilename = 'motorstate.csv'): # later may be add [mode = 'all/torque/position']  
    ''' Open MatPlotLib window with realtime graph
    Args: 
        csvfilename (str, optional): name of data source file 

    Return: 
        None: that function does not return any value. It show graphs
    '''
    index = count()
    ...
    def animate(i):
        ...
        data = read_last_n_lines_with_tail(csvfilename, 700) # read only n strings (time window). If n=0 read full file
